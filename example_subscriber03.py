...
def called2(msg):
    rospy.loginfo("sub02 %s ", msg.data)
    pub = rospy.Publisher('hello_subscriber03', String, queue_size=10)
    pub.publish(msg.data)

if __name__=="__main__":
    rospy.init_node("third_subscriber_node")
    # sub=rospy.Subscriber('helloworld', String, callback=called)
    # rate=rospy.spin()
    sub=rospy.Subscriber('helloworld02', String, callback=called2)
    sub=rospy.Subscriber('helloworld03', String, callback=called2)
    rate=rospy.spin()
